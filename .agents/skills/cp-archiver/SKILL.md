...
题目名称作为目录名时，格式为 `<OJ前缀>_<题号>_<题名>`，前缀规范见 `references/oj_prefixes.md`。**遇到表中没有的 OJ 时，先确定前缀并追加到该文件中**，再创建目录。替换非法字符：`空格→_` `/\:*?"<>|` 删除。
...
- 行内变量、下标、公式、复杂度和数学关系使用单个 `$` 包裹，如 `$a_i$`、`$O(n \log n)$`、`$1 \le i \le n$`。
- 较长或需要独立展示的公式使用 `$$` 包裹，并让公式单独成行：
  ...
  $$
  ```
- 代码块、行内代码、文件名和命令保持原格式，不使用数学公式标记。
