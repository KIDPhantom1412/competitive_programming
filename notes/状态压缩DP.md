...
组合压缩不必考虑顺序，因此递推的时候有一个技巧：最低位递推优化。
...
一般是二维网格上的DP问题。插头DP也可看做一种轮廓线DP。

### 行递推轮廓线DP

通常来说轮廓线是每次移动一格，但是有的轮廓线DP问题能简化成按列递推。

行递推也不一定是行，也可以是列，可以根据时间复杂度来选择行或者列。当根据行递推时，一行的状态数会达到$2^n$（如果一行有$n$个元素），这是非常大的数字，我们可以选择比较短的一边作为递推的方向。
